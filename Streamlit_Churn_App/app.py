...
with tab1:
    ...
    with col1_b:
        tenure = st.number_input("Masa Berlangganan / Tenure (Bulan)", min_value=0, value=1)
        address_count = st.number_input("Jumlah Alamat Terdaftar (NumberOfAddress)", min_value=1, value=1)
        warehouse_dist = st.number_input("Jarak Gudang ke Rumah / WarehouseToHome (km)", min_value=1, value=10)
...
with tab2:
    ...
    with col2_b:
        device_reg = st.number_input("Jumlah Perangkat Terdaftar (NumberOfDeviceRegistered)", min_value=1, value=1)
        hour_spend = st.number_input("Jam Dihabiskan di Aplikasi (HourSpendOnApp)", min_value=0.0, value=2.0)
...
with tab3:
    ...
    with col3_a:
        ...
        order_count = st.number_input("Total Pesanan (OrderCount)", min_value=1, value=1)
        order_hike = st.number_input("Kenaikan Jumlah Pesanan dari Tahun Lalu (%)", min_value=0.0, value=15.0)
        coupon_used = st.number_input("Kupon Digunakan (CouponUsed)", min_value=0, value=1)
    with col3_b:
        day_since_order = st.number_input("Hari Sejak Pesanan Terakhir (DaySinceLastOrder)", min_value=0, value=5)
        cashback = st.number_input("Rata-rata Cashback (CashbackAmount)", min_value=0.0, value=150.0)
        satisfaction = st.slider("Skor Kepuasan (SatisfactionScore)", min_value=1, max_value=5, value=3)
        complain = st.selectbox("Riwayat Komplain (Complain)", ["Tidak Ada", "Ada"])
        complain_val = 1 if complain == "Ada" else 0

st.divider()

# 5. Logika Tombol Prediksi dan Penyesuaian Fitur
if st.button("Jalankan Prediksi Risiko", type="primary", use_container_width=True):
    
    # Menyusun data input mentah sesuai urutan 18 kolom
    input_dict = {
        'Tenure': [tenure],
    ...
    try:
        # Menerapkan One-Hot Encoding pada input pengguna
        input_encoded = pd.get_dummies(input_df)
        
        # Mengekstrak daftar kolom yang direkam oleh model saat pelatihan
        model_features = model.feature_names_in_
        
        # Menyelaraskan kolom input dengan kolom model (Reindexing)
        # Kolom dummy yang tidak ada pada input akan diisi nilai 0
